...
public final class GenomicsDBImportIntegrationTest extends CommandLineProgramTest {
    ...
    public void testGenomicsDBImportFileInputsAgainstCombineGVCF() throws IOException {
        testGenomicsDBAgainstCombineGVCFs(LOCAL_GVCFS, INTERVAL, b38_reference_20_21);
    }

    @Test
    public void testGenomicsDBImportFileInputsAgainstCombineGVCFWithNonDiploidData() throws IOException {
        testGenomicsDBAgainstCombineGVCFs(Arrays.asList(NA12878_HG37, MULTIPLOID_DATA_HG37), INTERVAL_NONDIPLOID, b37_reference_20_21);
    }
    ...
    private File runCombineGVCFs(final List<String> inputs, final SimpleInterval interval, final String reference) {
    ...
    private void testGenomicsDBAgainstCombineGVCFs(final List<String> vcfInputs, final SimpleInterval interval, final String referenceFile) throws IOException {
        final String workspace = createTempDir("genomicsdb-tests-").getAbsolutePath() + "/workspace";

        writeToGenomicsDB(vcfInputs, interval, workspace, 0, false, 0, 1);
        checkJSONFilesAreWritten(workspace);
        File expectedCombinedVCF = runCombineGVCFs(vcfInputs, interval, referenceFile);
    ...
    // TODO this test should be enabled when the genomicsDB is updated to no longer require absolute paths
    @Test (enabled = false)
