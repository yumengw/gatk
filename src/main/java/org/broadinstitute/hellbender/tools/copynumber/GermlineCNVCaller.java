...
/**
 * Call copy-number variants in germline samples given their fragment counts and the corresponding output of
 ...
 * <h3>Introduction</h3>
 *
 * <p>Reliable detection of copy-number variation (CNV) from read-depth ("coverage") data such as whole exome sequencing
 * (WES), whole genome sequencing (WGS), and gene panel coverage profiles requires a comprehensive model of library
 * preparation and sequencing biases. The probabilistic model and the associated inference scheme implemented in
 * {@link GermlineCNVCaller} includes provisions for correcting for average location-specific biases, as well as
 * sample-to-sample coverage variation and the location-specific and sample-specific coverage variance.</p>
 *
 ...
 * kit). We refer to this run-mode as the <b>COHORT</b> mode. The number of samples required for the COHORT mode
 * depends on many factors such as the quality of sequenced samples and the stringency of following the
 * library preparation and sequencing protocols. For WES and WGS, we recommend at least 30 samples for the cohort
 * mode.</p>
 *
 ...
 * model parameters. We refer to this run-mode as the <b>CASE</b> mode. There is no lower limit on the number of
 * samples for running {@link GermlineCNVCaller} in the case mode.</p>
 *
 * <p>In both run-modes, the output calls of {@link DetermineGermlineContigPloidy} are required for all samples.
 ...
 *     <dd><p>The tool can be run in the COHORT mode by using the argument <code>--run-mode COHORT</code>.
 *      In this run-mode, coverage model parameters are inferred simultaneously with the CNV events. Depending on
 ...
 *      CNV calling in one or more similarly-sequenced samples (see below). If a previously obtained coverage model
 *      parameter bundle is provided via <code>--model &lt;previous_model_path&gt;</code>, those parameters will be
 *      used for initialization and a new parameter bundle will be generated based on the provided cohort.
 ...
 *
 *     <dt>CASE run-mode</dt>
 *     <dd><p>The tool can be run in the CASE mode by using the argument <code>--run-mode CASE</code>. The path to a
 ...
 *      arguments will be ignored. The output of the CASE run-mode is only the "-calls" subdirectory.</p>
 * </dl>
 *
 ...
 * model hyperparameters, such as the prior probability of alternative CNV states, active regions, the coherence
 * length of CNV events and active/silent domains, and the typical scale of interval- and sample-specific unexplained
 * variance. These hyperparameters are not universal and must be properly tuned for each sequencing protocol.</p></dd>
 *
