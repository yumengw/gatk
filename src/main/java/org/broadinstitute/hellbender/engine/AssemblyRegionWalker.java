...
public abstract class AssemblyRegionWalker extends GATKTool {
    ...
    @Advanced
    @Argument(fullName=SHARD_SIZE_LONG_NAME, doc = "Maximum size of each read shard, in bases. Set to " + NO_INTERVAL_SHARDING + " for one shard per interval (or one shard per contig, if intervals are not explicitly specified). For good performance, this should typically be much larger than the maximum assembly region size.", optional = true)
    protected int readShardSize = defaultReadShardSize();

    @Advanced
    @Argument(fullName=SHARD_PADDING_LONG_NAME, doc = "Each read shard has this many bases of extra context on each side. Read shards must have as much or more padding than assembly regions.", optional = true)
